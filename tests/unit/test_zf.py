...
def test_zf_create_new_zone(app_new, zfzone_common_data):
    ...
    with app_new.app_context():
        zone = create_zone(zone_name = zone_dns_name, soa_rrset = soa_rrset, ns_rrset = ns_rrset)
...
def test_zf_create_existing_zone(app_with_single_zone, zfzone_common_data):
    ...
    try:
        with app_with_single_zone.app_context():
            _ = create_zone(zone_name = zone_dns_name, soa_rrset = soa_rrset, ns_rrset = ns_rrset)
        assert False
    except Forbidden:
        assert True
...
def test_zf_delete_zone(app_with_single_zone, zfzone_common_data):
    ...
    with app_with_single_zone.app_context():
        ...
        expected_filepath = os.path.join(app_with_single_zone.config['ZONE_FILE_FOLDER'], f"{zone_dns_name}zone")
    assert deleted
...
def test_zf_create_a_record(app_with_single_zone, zfzone_common_data):
    ...
    with app_with_single_zone.app_context():
        created_record = create_record(record_name = record_name, record_type = record_type, record_data = record_data, zone_name= zone_name)
    assert isinstance(created_record, dns.rrset.RRset)
    assert created_record.name == record_dns_name.relativize(origin = zfzone_common_data.origin)
    ...
    for key in record_data.keys():
        assert getattr(created_rdata, key) == record_data[key]

    with app_with_single_zone.app_context():
        got_records = get_records(zone_name = zone_name, record_name = record_name, record_type = record_type)
...
def test_zf_create_record_nowrite(app_with_single_zone, zfzone_common_data):
    ...
    with app_with_single_zone.app_context():
        created_record = create_record(record_name = record_name, record_type = record_type, record_data = record_data, zone_name= zone_name, write = False)
    assert isinstance(created_record, dns.rrset.RRset)
    assert created_record.name == record_dns_name.relativize(origin = zfzone_common_data.origin)
    assert created_record.rdtype == dns.rdatatype.from_text(record_type)
    created_rdata = created_record[0]
    ...
    try:
        with app_with_single_zone.app_context():
            _ = get_records(zone_name = zone_name, record_name = record_name, record_type = record_type)
        assert False
    except NotFound:
        assert True
...
def test_zf_update_a_record(app_with_single_zone, zfzone_common_data):
    ...
    with app_with_single_zone.app_context():
        existing_records = get_records(zone_name = zone_name, record_name = record_name, record_type = record_type)
    for key in record_data.keys():
        assert record_data[key] != getattr(existing_records[0][0], key)

    with app_with_single_zone.app_context():
        updated_record = update_record(record_name = record_name, record_type = record_type, record_data = record_data, zone_name= zone_name)
    assert isinstance(updated_record, dns.rrset.RRset)
    updated_rdata = updated_record[0]
    for key in record_data.keys():
        assert getattr(updated_rdata, key) == record_data[key]
...
def test_zf_delete_a_record(app_with_single_zone, zfzone_common_data):
    ...
    with app_with_single_zone.app_context():
        _ = delete_record(record_name = record_name, record_type = record_type, record_data = record_data, record_ttl = record_ttl, zone_name = zone_name)
