...
from zoneforge.core import get_records, create_record, update_record, delete_record, record_to_response
from werkzeug.exceptions import *

api = Namespace('records', description='DNS record related operations', path='/')

record_get_parser = reqparse.RequestParser()
record_get_parser.add_argument('name', type=str, help='Name of the DNS record', required=False)
record_get_parser.add_argument('type', type=str, help='Type of DNS Record', required=False)

record_post_parser = record_get_parser.copy()
record_post_parser.replace_argument('name', type=str, help='Name of the DNS record', required=True)
record_post_parser.replace_argument('type', type=str, help='Type of DNS Record', required=True)
record_post_parser.add_argument('ttl', type=str, help='TTL of the DNS record', required=False)
record_post_parser.add_argument('data', type=dict, help='RData for the DNS Record', required=True)
record_post_parser.add_argument('comment', type=str, help='Comment for the DNS Record', required=False)

record_put_parser = record_get_parser.copy()
record_put_parser.replace_argument('type', type=str, help='Type of DNS Record', required=True)
record_put_parser.add_argument('ttl', type=str, help='TTL of the DNS record', required=False)
record_put_parser.add_argument('data', type=dict, help='RData for the DNS Record', required=True)
record_put_parser.add_argument('comment', type=str, help='Comment for the DNS Record', required=False)

record_delete_parser = record_get_parser.copy()
record_delete_parser.replace_argument('type', type=str, help='Type of DNS Record', required=True)
record_delete_parser.add_argument('ttl', type=str, help='TTL of the DNS Record', required=True)
record_delete_parser.add_argument('data', type=dict, help='RData of the DNS Record', required=True)

dns_fields_model = api.model('DnsRecordFields', {
    '*': fields.Wildcard(fields.Raw)
})
dns_record_model = api.model('DnsRecord', {
...
class DnsRecord(Resource):
    ...
    def post(self, zone_name: str):
        ...
        new_record = create_record(zone_name=zone_name, record_name=args['name'], record_type=args['type'], record_ttl=args['ttl'], record_data=args['data'], record_comment=args['comment'])
