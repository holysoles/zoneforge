...
from zoneforge.core import get_zones, create_zone, delete_zone, create_record, update_record, friendly_email_to_zone_format
...
zone_parser.replace_argument('name', type=str, help='Name of the DNS Zone', required=True)
zone_post_parser.add_argument('soa_ttl', type=str, help='SOA TTL', required=True) 
zone_post_parser.add_argument('admin_email', type=str, help='Email address of zone admin', required=True) 
zone_post_parser.add_argument('refresh', type=str, help='Record refresh frequency for secondary nameservers', required=True)
zone_post_parser.add_argument('retry', type=str, help='Retry frequency for failed zone transfers', required=True) 
zone_post_parser.add_argument('expire', type=str, help='Period after which the zone is discarded if no updates are received', required=True) 
zone_post_parser.add_argument('minimum', type=str, help='Used for calculation of negative response TTL', required=True)
zone_post_parser.add_argument('primary_ns', type=str, help='Primary nameserver for the zone', required=True) 
zone_post_parser.add_argument('primary_ns_ttl', type=str, help="TTL for the primary nameserver's NS record", required=True)
zone_post_parser.add_argument('primary_ns_ip', type=str, help="IP for the primary nameserver's A record", required=False)
zone_post_parser.add_argument('primary_ns_a_ttl', type=str, help="TTL for the primary nameserver's A record", required=False) 
...
zone_put_parser.add_argument('admin_email', type=str, help='Email address of zone admin', required=True) 
zone_put_parser.add_argument('refresh', type=str, help='Record refresh frequency for secondary nameservers', required=True)
zone_put_parser.add_argument('retry', type=str, help='Retry frequency for failed zone transfers', required=True) 
zone_put_parser.add_argument('expire', type=str, help='Period after which the zone is discarded if no updates are received', required=True) 
zone_put_parser.add_argument('minimum', type=str, help='Used for calculation of negative response TTL', required=True)
zone_put_parser.add_argument('primary_ns', type=str, help='Primary nameserver for the zone', required=True)
